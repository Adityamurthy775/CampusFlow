...
function Donut({ segments, total, center }) {
  let cursor = 0;
  const stops = segments.map((segment) => { const start = cursor; cursor += segment.value; return `${segment.color} ${start}% ${cursor}%`; }).join(",");
...
export default function Dashboard() {
  ...
  const active = featureNav.some(([id]) => id === path.split("/")[2]) ? path.split("/")[2] : "overview";
