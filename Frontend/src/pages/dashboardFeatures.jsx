...
function DetailModal({ detail, onClose }) {
  ...
  return <div className="portal-modal-backdrop" role="presentation" onClick={onClose}><section className="portal-modal" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><button className="portal-modal-close" onClick={onClose} aria-label="Close">Ã—</button><span className="portal-modal-kicker">{isAssignment ? "Assignment details" : "Announcement details"}</span><h2>{item.name || item.title}</h2><p className="portal-modal-meta">{isAssignment ? `${item.subjectinfo?.name || "Subject"} · Due ${date(item.duedate)}` : `${item.postedby?.username || "CampusFlow"} · ${date(item.createdAt)}`}</p><p>{isAssignment ? item.descp : item.content}</p>{isAssignment && item.instructions && <div className="portal-modal-section"><strong>Instructions</strong><p>{item.instructions}</p></div>}{isAssignment && <div className="portal-modal-facts"><span><b>Maximum marks</b>{item.maxmarks || "â€”"}</span><span><b>Status</b>{item.status || "â€”"}</span><span><b>Teacher</b>{item.teacherinfo?.username || "â€”"}</span></div>}</section></div>;
}

function Table({ headers, rows, empty = "No records found." }) {
  if (!rows.length) return <Empty>{empty}</Empty>;
  return <div className="portal-data-table"><table><thead><tr>{headers.map((header) => <th key={header}>{header}</th>)}</tr></thead><tbody>{rows.map((row, index) => <tr key={row.key || index}>{headers.map((header) => <td key={header}>{row[header] ?? "â€”"}</td>)}</tr>)}</tbody></table></div>;
}
