...
export default function DashboardFeatures({ active, role, user, onNotice }) {
  ...
  const ownedSubjects = role === "teacher"
  ...
  if (active === "schedule") {
    ...
    return <div className="portal-workspace"><section className="portal-card"><div className="portal-section-heading"><div><h2>Weekly Schedule</h2><p>{data.schedule?.branch} · Year {data.schedule?.year} · Semester {data.schedule?.semester}</p></div></div><div className="portal-schedule-days">{days.map((day) => <article key={day.day}><h3>{day.day}</h3>{day.periods.map((period, index) => <div key={`${period.subjectinfo?.code}-${index}`}><time>{period.start} – {period.end}</time><strong>{period.subjectinfo?.name || period.subjectName}</strong><small>{period.roominfo?.code || period.roomCode || "Room TBD"} · {period.faculty?.[0]?.designation || "Faculty TBD"}</small></div>)}</article>)}</div></section></div>;
  }

  if (active === "assignments") {
    const canCreate = ["teacher", "hod", "admin"].includes(role);
    const mySubmission = (assignmentId) => data.submissions?.find((item) => (item.assignment?._id || item.assignment) === assignmentId);
    return <div className="portal-workspace"><div className="portal-workspace-grid"><section className="portal-card"><div className="portal-section-heading"><div><h2>Assignments</h2><p>Create, download, submit, and track work.</p></div></div><div className="portal-card-list">{data.assignments?.map((assignment) => { const submission = mySubmission(assignment._id); return <article key={assignment._id}><div><strong>{assignment.name}</strong><small>{assignment.subjectinfo?.name} · Due {date(assignment.duedate)}</small></div><span>{assignment.status}</span><p>{!submission ? "Not submitted" : submission.marksobtained == null ? "Awaiting grade" : `${submission.marksobtained}/${submission.assignment?.maxmarks || "—"}`}</p><div>{assignment.attachments?.map((attachment, index) => <button key={attachment.storageName} onClick={() => api.download(`/assignment-api/download/${assignment._id}/${index}`, attachment.originalName)}>Download brief</button>)}{submission && <button onClick={() => api.download(`/submission-api/download/${submission._id}/0`, submission.attachments?.[0]?.originalName || "submission")}>Download submission</button>}</div></article>; })}</div>{!data.assignments?.length && <Empty />}</section>{canCreate && <section className="portal-card"><div className="portal-section-heading"><div><h2>Upload Assignment</h2><p>Share the brief and deadline with your subject.</p></div></div><form onSubmit={(event) => { event.preventDefault(); const body = new FormData(); Object.entries(form).forEach(([key, value]) => body.append(key, value)); if (file) body.append("file", file); submit("/assignment-api/create", body, "Assignment published successfully", true); }}><Field label="Title"><input required value={form.name || ""} onChange={set("name")} /></Field><Field label="Subject"><select required value={form.subjectinfo || ""} onChange={set("subjectinfo")}><option value="">Select subject</option>{ownedSubjects.map((subject) => <option value={subject._id} key={subject._id}>{subject.code} · {subject.name}</option>)}</select></Field><Field label="Description"><textarea required value={form.descp || ""} onChange={set("descp")} /></Field><Field label="Instructions"><textarea value={form.instructions || ""} onChange={set("instructions")} /></Field><div className="portal-form-row"><Field label="Due date"><input required type="date" value={form.duedate || ""} onChange={set("duedate")} /></Field><Field label="Maximum marks"><input min="1" type="number" value={form.maxmarks || ""} onChange={set("maxmarks")} /></Field></div><Field label="Assignment file (PDF, Office, image, text, or ZIP upto 10 MB)"><input required type="file" onChange={(event) => setFile(event.target.files[0] || null)} /></Field><button className="portal-primary-action" disabled={busy}>{busy ? "Publishing…" : "Publish assignment"}</button></form></section>}{role === "student" && <section className="portal-card"><div className="portal-section-heading"><div><h2>Submit Assignment</h2><p>Upload your answer file for an open assignment.</p></div></div><form onSubmit={(event) => { event.preventDefault(); if (!file) return setError("Choose a submission file"); const body = new FormData(); body.append("file", file); submit(`/submission-api/create/${form.assignmentId}`, body, "Assignment submitted successfully", true); }}><Field label="Assignment"><select required value={form.assignmentId || ""} onChange={set("assignmentId")}><option value="">Select assignment</option>{data.assignments?.map((assignment) => <option value={assignment._id} key={assignment._id}>{assignment.name}</option>)}</select></Field><Field label="Answer file (upto 10 MB)"><input required type="file" onChange={(event) => setFile(event.target.files[0] || null)} /></Field><button className="portal-primary-action" disabled={busy || !form.assignmentId}>{busy ? "Submitting…" : "Submit assignment"}</button></form></section>}</div></div>;
  }

  if (active === "attendance") {
    const canMark = ["teacher", "hod", "admin"].includes(role);
    const markable = role === "teacher" ? ownedSubjects : data.subjects || [];
  ...
  if (active === "grades") {
    const canReview = ["teacher", "hod", "admin"].includes(role);
  ...
  if (active === "events") {
    ...
    return <div className="portal-workspace"><div className="portal-workspace-grid"><section className="portal-card"><div className="portal-section-heading"><div><h2>Campus Events</h2><p>Academic, cultural, sports, placement, and exam events.</p></div></div><div className="portal-card-list">{data.events?.map((event) => <article key={event._id}><div><strong>{event.name}</strong><small>{event.catogery} · {date(event.startdate)} – {date(event.enddate)}</small></div><span>{event.status}</span><p>{event.decp}</p></article>)}{!data.events?.length && <Empty />}</div></section><section className="portal-card"><div className="portal-section-heading"><div><h2>Create Event</h2><p>Student events enter the review queue.</p></div></div><form onSubmit={(event) => { event.preventDefault(); submit("/event-api/create", { ...form, catogery: form.catogery || "academic", members: Number(form.members) || 0, status: canPublish ? "published" : "pending" }, "Event created successfully"); }}><Field label="Event name"><input required value={form.name || ""} onChange={set("name")} /></Field><Field label="Category"><select required value={form.catogery || "academic"} onChange={set("catogery")}><option value="academic">Academic</option><option value="cultural">Cultural</option><option value="sports">Sports</option><option value="placement">Placement</option><option value="holiday">Holiday</option><option value="exam">Exam</option><option value="seminar">Seminar</option><option value="other">Other</option></select></Field><Field label="Description"><textarea required value={form.decp || ""} onChange={set("decp")} /></Field><div className="portal-form-row"><Field label="Start date"><input required type="date" value={form.startdate || ""} onChange={set("startdate")} /></Field><Field label="End date"><input required type="date" value={form.enddate || ""} onChange={set("enddate")} /></Field><Field label="Members"><input min="0" type="number" value={form.members || 0} onChange={set("members")} /></Field></div><button className="portal-primary-action" disabled={busy}>{busy ? "Publishing…" : "Create event"}</button></form></section></div></div>;
  }

  if (active === "announcements") {
    const canCreate = ["teacher", "hod", "admin", "placement-office"].includes(role);
    return <div className="portal-workspace"><div className="portal-workspace-grid"><section className="portal-card"><div className="portal-section-heading"><div><h2>Announcements</h2><p>Campus-wide and role updates.</p></div></div><div className="portal-card-list">{data.announcements?.map((announcement) => <article key={announcement._id}><div><strong>{announcement.name}</strong><small>{announcement.postedby?.username} · {date(announcement.createdAt)}</small></div><span>{announcement.priority}</span><p>{announcement.content}</p></article>)}{!data.announcements?.length && <Empty />}</div></section><section className="portal-card"><div className="portal-section-heading"><div><h2>Post Announcement</h2><p>Visible to every authenticated role.</p></div></div>{canCreate ? <form onSubmit={(event) => { event.preventDefault(); submit("/announcement-api/create", form, "Announcement published successfully"); }}><Field label="Title"><input required value={form.name || ""} onChange={set("name")} /></Field><Field label="Message"><textarea required value={form.content || ""} onChange={set("content")} /></Field><Field label="Priority"><select value={form.priority || "normal"} onChange={set("priority")}><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="urgent">Urgent</option></select></Field><button className="portal-primary-action" disabled={busy}>{busy ? "Publishing…" : "Publish announcement"}</button></form> : <Empty>Only faculty, HOD, placement office, and admin can publish announcements.</Empty>}</section></div></div>;
  }
